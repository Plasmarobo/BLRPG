...
def put_req(output, sk_id, rtype, rname, rval)
  output << "Prerequisite.create(skill_template_id: #{sk_id}.id, prereq_type: '#{rtype}', prereq_name: '#{rname}', value: #{rval})\n"
end
        ...
        row.each do |reference|
          if (level = /Level\s*(\d*)/.match(reference)) != nil
            ...
            if reference != nil and reference != "nil"
              # This could be improved by setting value to the ID of 
